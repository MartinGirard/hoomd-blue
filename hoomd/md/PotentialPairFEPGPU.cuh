...
#ifndef __POTENTIAL_PAIR_FEP_GPU_CUH__
#define __POTENTIAL_PAIR_FEP_GPU_CUH__
...
struct pair_fep_args_t
    {
    //! Construct a pair_args_t
    pair_fep_args_t(Scalar4* _d_force,
                ...
                const Scalar _charge_override,
                const GPUPartition& _gpu_partition,
                const hipDeviceProp_t& _devprop)
        : d_force(_d_force), N(_N), n_max(_n_max),
          ...
          threads_per_particle(_threads_per_particle), type_override(_type_override),
          charge_override(_charge_override), gpu_partition(_gpu_partition),
          devprop(_devprop) {};
...
__global__ void
gpu_compute_pair_fep_shared_kernel(Scalar4* d_force,
    ...
    if (active)
        ...
        if (evaluator::needsCharge())
            qi = charge_override == INFINITY? __ldg(d_charge + idx) : charge_override;
...
struct PairFEPForceComputeKernel
    {
    ...
    static void launch(const pair_fep_args_t& pair_args,
        ...
        if (tpp == pair_args.threads_per_particle)
            ...
            max_block_size
                = get_max_block_size(gpu_compute_pair_fep_forces_shared_kernel<evaluator,
                                                                           shift_mode,
                                                                           compute_virial,
                                                                           tpp,
                                                                           true>);
            ...
            hipFuncGetAttributes(
                &attr,
                reinterpret_cast<const void*>(&gpu_compute_pair_fep_forces_shared_kernel<evaluator,
                                                                                     shift_mode,
                                                                                     compute_virial,
                                                                                     tpp,
                                                                                     true>));
            ...
            if (enable_shared_cache)
                {
                hipLaunchKernelGGL((gpu_compute_pair_fep_forces_shared_kernel<evaluator,
                                                                          shift_mode,
                                                                          compute_virial,
                                                                          tpp,
                                                                          true>),
            ...
            {
            PairFEPForceComputeKernel<evaluator, shift_mode, compute_virial, tpp / 2>::launch(
                pair_args,
                range,
                d_params);
            }
...
struct PairFEPForceComputeKernel<evaluator, shift_mode, compute_virial, 0>
    {
    static void launch(const pair_fep_args_t& pair_args,
                       std::pair<unsigned int, unsigned int> range,
                       const typename evaluator::param_type* d_params)
...
gpu_compute_pair_fep_forces(const pair_fep_args_t& pair_args,
                        const typename evaluator::param_type* d_params)
    {
    assert(d_params);
    ...
    for (int idev = pair_args.gpu_partition.getNumActiveGPUs() - 1; idev >= 0; --idev)
        ...
        if (pair_args.compute_virial)
            ...
            case 0:
                {
                PairFEPForceComputeKernel<evaluator, 0, 1, gpu_pair_force_max_tpp>::launch(pair_args,
                                                                                        range,
                                                                                        d_params);
                break;
                }
            case 1:
                {
                PairFEPForceComputeKernel<evaluator, 1, 1, gpu_pair_force_max_tpp>::launch(pair_args,
                                                                                        range,
                                                                                        d_params);
                break;
                }
            case 2:
                {
                PairFEPForceComputeKernel<evaluator, 2, 1, gpu_pair_force_max_tpp>::launch(pair_args,
                                                                                        range,
                                                                                        d_params);
                break;
                }
            ...
            case 0:
                {
                PairFEPForceComputeKernel<evaluator, 0, 0, gpu_pair_force_max_tpp>::launch(pair_args,
                                                                                        range,
                                                                                        d_params);
                break;
                }
            case 1:
                {
                PairFEPForceComputeKernel<evaluator, 1, 0, gpu_pair_force_max_tpp>::launch(pair_args,
                                                                                        range,
                                                                                        d_params);
                break;
                }
            case 2:
                {
                PairFEPForceComputeKernel<evaluator, 2, 0, gpu_pair_force_max_tpp>::launch(pair_args,
                                                                                        range,
                                                                                        d_params);
                break;
                }
